...
const sanitizeBankName = (bankName) => {
  if (!bankName) return "Corporate Bank";
  return bankName
    .replace(/Standard Chartered/gi, "Corporate Bank")
    .replace(/SCB/gi, "Corporate Bank")
    .replace(/scb/gi, "Corporate Bank");
};
...
function PayrollManual() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("authToken");
    fetch(`http://localhost:8081/api/accounts/${filters.debitAccount}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
  ...
  useEffect(() => {
    if (filters.payrollType === "Domestic") {
      ...
      setNewEmployee((prev) => ({
        ...prev,
        currency: filters.paymentCurrency,
      }));
    }
  ...
  const fetchBatchGrid = () => {
    const token = localStorage.getItem("authToken");
    fetch("http://localhost:8081/api/us3/batch/grid", {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
  ...
  const handleFiltersChange = (updated) => {
    ...
    if (updated.debitAccount) {
      const acc = accounts.find(
        (a) => String(a.id) === String(updated.debitAccount),
      );
  ...
  const t1 = selectedCurrency
    ? INR_THRESHOLD_1 / (INR_PER[selectedCurrency] || 1)
    : 0;
  const t2 = selectedCurrency
    ? INR_THRESHOLD_2 / (INR_PER[selectedCurrency] || 1)
    : 0;
  ...
  const availableBalance = accountDetails
    ? Number(accountDetails.balance).toLocaleString("en-IN", {
  ...
  const handleSubmitForApproval = () => {
    ...
    const batchData = {
      ...
      maxDebitAmount: Math.max(
        ...employees.map((emp) => Number(emp.salary) || 0),
      ),
    ...
    fetch("http://localhost:8081/api/us3/batch", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
        ...
        <div className="p-4 bg-white shadow-sm rounded-3">
          <header className="pb-3 mb-4 border-bottom">
            ...
            <p className="text-muted">
              Create payment batches by entering employee details manually.
            </p>
          </header>
          ...
          {accountDetails && (
            <div className="row mb-3" style={{ minHeight: 285 }}>
              <div
                className="col-md-5 col-lg-4 d-flex align-items-stretch"
                style={{ paddingLeft: 0 }}
              >
                <div
                  className="card p-4 shadow-sm h-100 border-primary"
                ...
                >
                  ...
                  <>
                    ...
                    <div>
                      <b>Bank:</b> {sanitizeBankName(accountDetails.bank)}
                    </div>
                    <div>
                        ...
                        {recentBatches.length === 0 ? (
                          ...
                          recentBatches.map((item, idx) => (
                            <li key={idx}>
                              ...
                              debited ({item.numberOfPayments} payments)
                            </li>
                          ))
                        )}
                      </ul>
          ...
          {!filters.debitAccount && (
            <div className="alert alert-info py-2">
              Please select a debit account to begin your payroll.
            </div>
          )}
