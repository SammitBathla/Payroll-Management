...
function PayrollList({
  ...
  const refreshAll = async () => {
    // Pending approvals
    const bRes = await fetch("http://localhost:8081/api/us5/approval/pending", {
      headers: {
    ...
    const pRes = await fetch("http://localhost:8081/api/us5/approval/processed", {
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
      }
  ...
  try {
    const endpoint = actionType === "approve"
      ? "http://localhost:8081/api/us5/approval/approve"
      : "http://localhost:8081/api/us5/approval/reject";
    const reqBody = {
